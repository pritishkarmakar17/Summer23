...
\item 
\color{red}{\textit{Orthogonal property} of different modes of HG beams }
\end{enumerate}
...
\begin{enumerate}
	...
	\item 
	As we will see later section, unlike the HG beam, LG bream carry orbital angular momentum due to its phase variation \textit{w.r.t.} $\phi$.
\end{enumerate}
\begin{figure}[!t]
...
% will do later\cite{beijers allen 93}
...
\begin{align}
	\expval{\boldsymbol{S}} =\frac{1}{\mu_0}\expval{\boldsymbol{\mathcal{E}} \times \boldsymbol{\mathcal{B}}} =\frac{1}{\mu_0}\; \frac{1}{2} \Re{\boldsymbol{E}^\ast \times \boldsymbol{B}} = 
	\frac{1}{2\mu_0}\:(\boldsymbol{E}^\ast \times \boldsymbol{B}+ \boldsymbol{E} \times \boldsymbol{B}^\ast )\label{eq:3.7}
\end{align}

From eq. \ref{eq:3.1} and \ref{eq:3.2} and using \ref{eq:3.6} and \ref{eq:3.7}, time-averaged linear momentum is given by
\begin{align}
	\boldsymbol{P} &= \int d\tau\: \expval{\boldsymbol{S}}\\
	&=\frac{\epsilon_0}{2}\int d\tau \:(\boldsymbol{E}^\ast \times \boldsymbol{B}+ \boldsymbol{E} \times \boldsymbol{B}^\ast )\nonumber\\
	&= \frac{\epsilon_0}{2i\omega}\int d\tau\: \left(\boldsymbol{E}^\ast \times (\nabla\times\boldsymbol{E}) -  \boldsymbol{E} \times (\nabla\times\boldsymbol{E}^\ast) \right)\\
\end{align}
...
\begin{align}
	\boldsymbol{j} &= \frac{1}{c^2}\boldsymbol{r}\times\boldsymbol{S} = \epsilon_0 \boldsymbol{r}\times(\boldsymbol{E} \times \boldsymbol{B})\label{eq:3.}\\
	\boldsymbol{J} &= \int \boldsymbol{j} \:d\tau\label{eq:3.4}
\end{align}

From eq. \ref{} and \ref{} and using \ref{} and \ref{}, time-averaged angular momentum, 
\begin{align}
	\boldsymbol{J} &= \int d\tau\: \boldsymbol{r}\times\expval{\boldsymbol{S}}\\
	&=\frac{\epsilon_0}{2}\int d\tau \:\boldsymbol{r}\times(\boldsymbol{E}^\ast \times \boldsymbol{B}+ \boldsymbol{E} \times \boldsymbol{B}^\ast )\nonumber\\
	&= \frac{\epsilon_0}{2i\omega}\int d\tau\: \boldsymbol{r}\times\left(\boldsymbol{E}^\ast \times (\nabla\times\boldsymbol{E}) -  \boldsymbol{E} \times (\nabla\times\boldsymbol{E}^\ast) \right)  \nonumber
\end{align}
...
Now according to \cite{bernatt allen 94}\cite{enk nien 92}, within the paraxial approximation to get time-averaged energy, linear momentum and angular momentum per unit length, $\mathcal{W}$, $\mathcal{P}$ and $\mathcal{J}$ respectively, along the beam propagating in z-direction, we have to integrate throughout the transverse xy-plane. So,
\begin{align}
	\mathcal{W}&=\frac{\epsilon_0}{2}\iint dx\:dy\: \boldsymbol{E}^\ast\cdot\boldsymbol{E}\\
	\mathcal{P}&=\frac{\epsilon_0}{2}\iint dx\:dy\: \boldsymbol{r}\times\expval{\boldsymbol{E}\times (\nabla\times\boldsymbol{E})}\\
	\mathcal{J}&=\frac{\epsilon_0}{2}\iint  dx\:dy\: \expval{\boldsymbol{E}\times (\nabla\times\boldsymbol{E})}
\end{align}
...
\begin{align}
	\boldsymbol{E}(x,y,z)=\boldsymbol{F}(x,y,z)\:e^{ikz} \label{}
\end{align}
...
From previous eq. \ref{} z-component of $\mathcal{J}$ for \ref{} is given by
...
\begin{align}
	...
	+\underbrace{\frac{\epsilon_0}{2i\omega}\iint dx\:dy\: (F_x^\ast F_y + F_y^\ast F_x)}_{\text{2nd term}}
\end{align}

Note that, in \ref{}, we can separately identify two terms. According to \cite{WO}, the first is associated with transverse distribution of electric field (amplitude and phase) as 
$x\frac{\partial}{\partial y} - y\frac{\partial}{\partial x}$
is equivalent to $\frac{\partial}{\partial\phi}$ in cylindrical coordinate and the second term to the polarization of electric field. Thus it is concludeed,\cite{enk nien 92} within the paraxial approximation, that
\begin{enumerate}
	\item 
	...
	\begin{align}
		\mathcal{S} = \frac{\epsilon_0}{2i\omega}\iint dx\:dy\: (F_x^\ast F_y + F_y^\ast F_x)
	\end{align}
\end{enumerate}
\subsection{Orbital Angular Momentum (OAM)}
Let envelop $\boldsymbol{F}$ corresponding to the EM wave \ref{} for a vortex beam (\textit{e.g.} LG beam) be,
\begin{align}
	\boldsymbol{F}(r,\phi,t)= u(r)\:\exp(il\phi)\:\Hat{\boldsymbol{p}}(t)
\end{align} where $e^{il\phi}$ refers to helical phase front, and $\Hat{\boldsymbol{p}} = p_x\Hat{x}+ p_y\Hat{y}+p_z\Hat{z}$ refers to unit polarization direction of electric field. Putting it in \ref{} considering $x\frac{\partial}{\partial y} - y\frac{\partial}{\partial x} \equiv \frac{\partial}{\partial\phi}$, we get,

\begin{align}
	\mathcal{L}&=\frac{\epsilon_0}{2i\omega} \iint r\:dr\:d\phi\:\left[F_\xi^\ast\frac{\partial}{\partial\phi}F_\xi\right]_{\xi=x,y}\nonumber\\
	&= 	\frac{\epsilon_0}{2\omega} l \left({p_x}^2+{p_y}^2\right)2\pi \int r\:dr\:\abs{u(r)}^2 \nonumber
\end{align}
considering z-component of $\Hat{\boldsymbol{p}}$ is smaller than transverse component by a factor of $w_0/l_d$, so ${p_x}^2+{p_y}^2 \approx {p_x}^2+{p_y}^2+{p_z}^2 = \abs{\Hat{\boldsymbol{p}}}^2 = 1$, then
\begin{align}
	\mathcal{L}=\frac{\pi\epsilon_0l}{\omega}\int r\:dr\:\abs{u(r)}^2 \label{}
\end{align}

Now measuring $\mathcal{W}$ we get,
\begin{align}
	\mathcal{W} &=\frac{\epsilon_0}{2}\iint r\:dr\:d\phi\: \boldsymbol{E}^\ast\cdot\boldsymbol{E}\nonumber\\
	&= \frac{\epsilon_0}{2}\iint r\:dr\:d\phi\: \boldsymbol{F}^\ast\cdot\boldsymbol{F}\nonumber\\
	&=\frac{\epsilon_0}{2} \left({p_x}^2+{p_y}^2+{p_z}^2\right) 2\pi\int r\:dr\:\abs{u(r)}^2\nonumber\\
	&=\pi\epsilon_0\int r\:dr\:\abs{u(r)}^2 \label{}
\end{align}

See from \ref{} and \ref{}, 
\begin{align}
	\boxed{\frac{\mathcal{L}}{\mathcal{W}} = \frac{l}{\omega}} \label{}
\end{align}

We know for a photon the energy associated with it is $\hbar\omega$, then from \ref{} we can say, the OAM associated with one photon is $l\hbar$.
